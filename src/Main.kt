...
fun temArvoreAdjacente(terreno: Array<Array<String?>>, coords: Pair<Int, Int>) : Boolean{
    ...
    if (linha > 0 && terreno[linha - 1][coluna] == "A") return true // acima
    if (coluna > 0 && terreno[linha][coluna - 1] == "A") return true // esquerda
    if (coluna < terreno[0].size - 1 && terreno[linha][coluna + 1] == "A") return true // direita
    if (linha < terreno.size - 1 && terreno[linha + 1][coluna] == "A") return true // abaixo
    return false
}
        ...
        for (numColuna in 0 until terreno[numLinha].size) {
            if (numColuna < terreno[0].size - 1) {
                ...
                if (numLinha < terreno.size - 1) {
                    ...
                    if (terreno[numLinha][numColuna] == arvore) resultado += " A${quebraDeLinha}"
                } else {
                    if (terreno[numLinha][numColuna] == null) resultado += espacos(1)
                    if (terreno[numLinha][numColuna] == tenda) resultado += " T"
                    if (terreno[numLinha][numColuna] == arvore) resultado += " A"
                }
            }
        }
        when (numLinha == terreno.size-1){
            true -> resultado += "$quebraDeLinha  ----------------------"
            else -> {}
        }
    }
    return resultado
...
fun terminouJogo(terreno: Array<Array<String?>>, contadoresVerticais: Array<Int?>, contadoresHorizontais: Array<Int?>): Boolean{
    for (numLinhas in 0 until contadoresHorizontais.size){
    ...
    for (numColunas in 0 until contadoresVerticais.size){
...
fun defineJogada(terreno: Array<Array<String?>>, pairCoordenadas: Pair<Int, Int>){
    if (apagaTenda == 1){
        terreno[pairCoordenadas.first][pairCoordenadas.second] = null
        apagaTenda = 0
    } else {
                                    ...
                                    if (coordenadas == saiString) {
                                        ...
                                        when (colocaTenda(terreno, pairCoordenadas)) {
                                            false -> println("Tenda nao pode ser colocada nestas coordenadas")
                                                ...
                                                defineJogada(terreno,pairCoordenadas)
                                                mapa = criaTerreno(terreno, contadoresV, contadoresH, Horizontal, Vertical)
                                                print(quebraDeLinha + mapa + quebraDeLinha)
                                            }
                                        }
                                    }
                                } while (!terminouJogo(terreno, contadoresV, contadoresH) && coordenadas != saiString)
